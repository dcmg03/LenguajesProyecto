...
#Define una clase MyTestCase que hereda de unittest.TestCase, donde se definen todas las pruebas.
class MyTestCase(unittest.TestCase):



    #Define un método setUp() que se ejecuta antes de cada prueba. En este caso, inicializa la aplicación Flask para pruebas.
    def setUp(self):
        # Inicializar la aplicación Flask para pruebas
        self.app = app.test_client()

    def test_fecha_corte_format(self):
        fecha_corte_match = re.search(fecha_corte_regex, test_text)
        self.assertIsNotNone(fecha_corte_match, "No se encontró la fecha de corte en el texto de la factura")
        if fecha_corte_match:
            fecha_corte = fecha_corte_match.group(1)
            # Verificar si la fecha de corte tiene el formato DD/MM/AAAA
            self.assertRegex(fecha_corte, r"\d{2}/\w{3}/\d{4}", "El formato de la fecha de corte no es válido")

    #Define un método de prueba que verifica si se puede acceder correctamente a la ruta principal '/' de la aplicación Flask y si devuelve un código de estado HTTP 200.
    ...
    #Define un método de prueba para validar expresiones regulares en un texto de factura.
    def test_regex_patterns(self):
        ...
        self.assertEqual(re.search(valor_total_regex, test_text).group(1), "26,990")
...
#Ejecuta las pruebas si el script es ejecutado directamente
